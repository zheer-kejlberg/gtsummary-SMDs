...
##### Then, we create the core function that takes the data and outputs the SMD results
...
##### Now, we create a function to clean the input data before use
...
##### This next function applies the clean_smd_data() and the core_smd_function(), modifying the output depending on 1) where the location of the SMDs (label or level) is to be, and 2) whether to print all pairwise SMDs or just SMDs between a reference group and all other groups
...
##### Now, four caller functions are defined to call the applied_smd_function() with preset values for ´location´ and for ´ref_group´
...
##### Finally, we just need to test it.

Here, we create 4 different unweighted tables:
